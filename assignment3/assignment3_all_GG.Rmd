...
I'm not sure if this accomplishes the goal of this problem. It creates a new data frame that lists each domain once and sums the variation locations corresponding to each domain. 
```{r}
aggregate(Swissvar_final2$VarLocation, by=list(Domain=Swissvar_final2$Domain), FUN=sum)
```

```{r}
Swisssvar_sums <- Swissvar_final2 %>%
  group_by(Domain) %>%
  summarise(sum = sum(VarLocation))%>%
  arrange(desc(sum))
head(Swisssvar_sums)
```
